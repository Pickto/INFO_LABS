...
def main():
    num = to_hexadecimal(-1.5, 4)
    print(num)
    num = from_hexadecimal(num, 4)
    print(num)
...
def to_hexadecimal(number, byte):
    ...
    return translate.translate_to_hex(sign + order + mantissa)
...
def from_hexadecimal(hex_number, byte):
    ...
    if byte == 8:
        ...
        bit = 8
        p = translate.translate_from(binary_number[1:9], 2) - 127
    res_number = translate.translate_from("1." + binary_number[bit + 1:],  2) * 2**p
